...
def compute_all_losses_for_model(model, x, mu, cov, p, intrinsic_dim):
    ...
    losses["Reconstruction"] = torch.sqrt(torch.mean(torch.linalg.vector_norm(x_recon - x, ord=2, dim=1) ** 2)).item()

    # 2. Tangent projection
    p_model = model.autoencoder.neural_orthogonal_projection(z)
    losses["Tangent penalty"] = torch.sqrt(torch.mean(torch.linalg.matrix_norm(p_model - p, ord="fro") ** 2)).item()
    ...
    losses["Ito penalty"] = torch.sqrt(torch.mean(torch.linalg.vector_norm(normal_component, ord=2, dim=1) ** 2)).item()
    ...
    penrose_mse = torch.mean(torch.linalg.matrix_norm(dpi - dphi_inv, ord="fro") ** 2)
    losses["Moore-Penrose error"] = torch.sqrt(penrose_mse).item()

    # 7. Diffeo penalty
    jacob_prod = torch.bmm(dpi, dphi)
    eye = torch.eye(dpi.shape[1], device=dpi.device).expand(dpi.shape[0], -1, -1)
    diffeo_mse = torch.mean(torch.linalg.matrix_norm(jacob_prod - eye, ord="fro") ** 2)
    losses["Diffeomorphism Error"] = torch.sqrt(diffeo_mse).item()
    ...
    losses["Ambient Cov Errors"] = torch.sqrt(torch.mean(ambient_cov_error)).item()

    # 9. Ambient drift error (direct extrinsic form from latent)
    ...
    # losses["Ambient Drift Errors"] = torch.sqrt(torch.mean(drift_err)).item()

    # 10. Ambient drift error (class method computation)
    ...
    losses["Ambient Drift Errors"] = torch.sqrt(torch.mean(drift_err2)).item()
