...
from ae.experiments.train_and_save import compare_mse, embedding_dim, embedding_matrix
...
# sympy expressions for ground truth curvature that can be numpified
manifold.riemann_curvature_tensor()
manifold.ricci_curvature_tensor()
manifold.scalar_curvature()
manifold.sectional_curvature(u, v) #sympy input variables
# Use the decoder probably to compute the curvature formula. We have jacobians and hessians
model = aedf_models["vanilla"]
# model local coordinates space isnt equal to ground truth local coordinates; need to compare the curvatures still
# get around this by taking the gt value corresponding to the parameter of hte GT curvature local coordinate, and
# encoded it.
model.autoencoder.decoder(z)
model.autoencoder.decoder_jacobian(z)
model.autoencoder.decoder_hessian(z)
